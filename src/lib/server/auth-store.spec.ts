...
vi.mock('./db', () => {
  ...
  return {
    db: {
      userAccount: {
        create: vi.fn(async ({ data }: { data: { username: string; tokens?: { create: { tokenHash: string } } } }) => {
          const user = {
        ...
        findUnique: vi.fn(async ({ where }: { where: { id?: string; username?: string }; select?: Record<string, boolean> }) => {
          const user = mockUsers.find(u => (where.id ? u.id === where.id : u.username === where.username));
          if (!user) return null;
      ...
      authToken: {
        ...
        findFirst: vi.fn(async ({ where, include }: { where: { tokenHash: string; revokedAt: null }; include?: { user: boolean } }) => {
          const token = mockTokens.find(t => t.tokenHash === where.tokenHash && t.revokedAt === null);
