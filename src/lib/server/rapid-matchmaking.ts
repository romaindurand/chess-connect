...
import { getOrCreateRapidPlayerRating } from './rapid-ranking-store';
...
async function removeExpiredData(reference: Date): Promise<void> {
	await db.rapidQueueEntry.deleteMany({
		where: {
			lastSeenAt: {
	...
	await db.rapidMatchProposal.deleteMany({
		where: {
...
async function hasOpenProposal(userId: string): Promise<boolean> {
	const proposal = await db.rapidMatchParticipant.findFirst({
		where: {
			userId,
			proposal: {
...
async function tryCreateProposalFor(userId: string): Promise<void> {
	...
	const selfQueue = await db.rapidQueueEntry.findUnique({
		where: { userId },
		include: {
			user: {
				include: { rapidRating: true }
			}
		}
	});
	if (!selfQueue || !selfQueue.user.rapidRating) {
		return;
	}

	const now = nowDate();
	const openCandidates = await db.rapidQueueEntry.findMany({
		where: {
			userId: { not: userId },
			user: {
				rapidRating: {
	...
	for (const candidate of openCandidates) {
		...
		if (
			!isCompatible(
				selfQueue.user.rapidRating.rating,
				selfRange,
				candidate.user.rapidRating.rating,
				candidateRange
			)
		) {
			continue;
		...
		await db.rapidMatchProposal.create({
			data: {
					...
					create: [
						{
							...
							ratingSnapshot: selfQueue.user.rapidRating.rating,
							ratingDeviation: selfQueue.user.rapidRating.ratingDeviation,
							joinedQueueAt: selfQueue.enteredAt
						},
						{
							userId: candidate.userId,
							ratingSnapshot: candidate.user.rapidRating.rating,
							ratingDeviation: candidate.user.rapidRating.ratingDeviation,
...
async function maybeStartRapidGame(proposalId: string): Promise<void> {
	const proposal = await db.rapidMatchProposal.findUnique({
		where: { id: proposalId },
		include: {
			participants: true
		}
	...
	const hostCreated = await createGame(host.username, RAPID_DEFAULTS);
	const joined = await joinGame(hostCreated.state.id, guest.username);

	await db.rapidMatchProposal.update({
...
export async function joinRapidQueue(userId: string): Promise<QueueView> {
	const now = nowDate();
	await removeExpiredData(now);
	await getOrCreateRapidPlayerRating(userId);

	const existing = await db.rapidQueueEntry.findUnique({ where: { userId } });
	if (!existing) {
		await db.rapidQueueEntry.create({ data: { userId, enteredAt: now, lastSeenAt: now } });
	} else {
		await db.rapidQueueEntry.update({ where: { userId }, data: { lastSeenAt: now } });
	}

	await tryCreateProposalFor(userId);
	return getRapidQueueView(userId);
}

export async function leaveRapidQueue(userId: string): Promise<void> {
	await db.rapidQueueEntry.deleteMany({ where: { userId } });
	await db.rapidMatchParticipant.updateMany({
		where: {
			userId,
...
export async function heartbeatRapidQueue(userId: string): Promise<QueueView> {
	const now = nowDate();
	await removeExpiredData(now);

	const queue = await db.rapidQueueEntry.findUnique({ where: { userId } });
	if (queue) {
		const waitSeconds = Math.max(0, Math.floor((now.getTime() - queue.enteredAt.getTime()) / 1000));
		await db.rapidQueueEntry.update({
			where: { userId },
			data: {
				lastSeenAt: now,
...
export async function getRapidQueueView(userId: string): Promise<QueueView> {
	const queue = await db.rapidQueueEntry.findUnique({
		where: { userId },
		select: { enteredAt: true, searchRange: true }
	});

	const proposalParticipant = await db.rapidMatchParticipant.findFirst({
		where: {
			userId,
...
export async function acceptOrRejectProposal(input: {
	...
	const participant = await db.rapidMatchParticipant.findFirst({
		where: {
	...
	if (!input.accept) {
		await db.rapidMatchParticipant.update({
			where: { id: participant.id },
			data: { rejectedAt: nowDate() }
		});
		await db.rapidQueueEntry.deleteMany({ where: { userId: input.userId } });
		const others = await db.rapidMatchParticipant.findMany({
			where: { proposalId: input.proposalId, userId: { not: input.userId }, rejectedAt: null }
		});
		for (const other of others) {
			const queue = await db.rapidQueueEntry.findUnique({ where: { userId: other.userId } });
			if (queue) {
				await db.rapidQueueEntry.update({
	...
	await maybeStartRapidGame(input.proposalId);

	const updated = await db.rapidMatchParticipant.findFirst({
