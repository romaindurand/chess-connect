...
<section class="mb-4 rounded border p-3 text-sm">
	{#if hasTimeControl}
		<p class="mb-2 text-xs text-gray-600">
			Cadence: {formatInitialControl(timeControlPerPlayerSeconds)} / joueur
		</p>
	{/if}

	<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
		<div
			class={`rounded border px-3 py-2 ${activeTurn === 'white' ? 'border-black' : 'border-transparent'}`}
		>
			<p class="font-medium">Blanc - {whiteScore}</p>
			<p class="text-gray-700">{whiteName}</p>
			{#if hasTimeControl}
				<p
					class={`mt-1 font-mono text-xs ${activeTurn === 'white' ? 'font-semibold text-black' : 'text-gray-700'}`}
				>
		...
		<div
			class={`rounded border px-3 py-2 ${activeTurn === 'black' ? 'border-black' : 'border-transparent'}`}
		>
			<p class="font-medium">Noir - {blackScore}</p>
			<p class="text-gray-700">{blackName}</p>
			{#if hasTimeControl}
				<p
					class={`mt-1 font-mono text-xs ${activeTurn === 'black' ? 'font-semibold text-black' : 'text-gray-700'}`}
				>
