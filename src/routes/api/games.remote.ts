...
const createGameSchema = z.object({
	name: z.string({ required_error: 'errors.nameRequired' })
		.min(2, 'errors.nameLength')
		.max(24, 'errors.nameLength'),
	timeLimitSeconds: z.number().int('errors.timeLimitInteger')
		.min(1, 'errors.timeLimitRange').max(1800, 'errors.timeLimitRange').optional(),
	incrementPerMoveSeconds: z.number().int('errors.incrementPerMoveInteger')
		.min(0, 'errors.incrementPerMoveRange').max(60, 'errors.incrementPerMoveRange').optional(),
	roundLimit: z.number().int('errors.roundLimitInteger')
		.refine(n => n > 0 && n % 2 !== 0, 'errors.roundLimitOddPositive').optional(),
	allowAiTrainingData: z.boolean({ invalid_type_error: 'errors.invalidAllowAiTrainingData' }).optional(),
	opponentType: z.enum(['human', 'ai'], { errorMap: () => ({ message: 'errors.invalidOpponentType' }) }).optional(),
	hostColor: z.enum(['white', 'black', 'random'], { errorMap: () => ({ message: 'errors.invalidHostColor' }) }).optional(),
	aiDifficulty: z.enum(['baseline'], { errorMap: () => ({ message: 'errors.invalidAiDifficulty' }) }).optional()
});

export const createGameRemote = command(createGameSchema, async (payload: z.infer<typeof createGameSchema>) => {
...
const actionPayloadSchema = z.discriminatedUnion('type', [
	z.object({
		type: z.literal('join'),
		name: z.string({ required_error: 'errors.nameRequired' })
			.min(2, 'errors.nameLength').max(24, 'errors.nameLength')
	}),
	z.object({
		type: z.literal('play'),
